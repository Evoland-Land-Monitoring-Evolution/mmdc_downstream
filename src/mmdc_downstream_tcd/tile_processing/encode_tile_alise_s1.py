...
def encode_tile_alise_s1(
    ...
    """
    transform = load_transform_one_mod(path_csv, mod="s1_asc").transform
    ...
    s1_asc_dates_final = pd.DataFrame(
        torch.load(os.path.join(folder_data, "s1_asc_dates.pt"))
    )["date_s1_asc"].values
    print(s1_asc_dates_final)

    reference_date = "2014-03-03"
    doy = torch.Tensor(
        (pd.to_datetime(s1_asc_dates_final) - pd.to_datetime(reference_date)).days
    )
    print(doy)
    tcd_values = None
    ...
    for enum, sliced in enumerate(slices_list):
        if not np.all(
            ...
            if np.all(
                ...
                data = prepare_patch_s1_malice(
                    folder_data, months_folders, sliced, s1_asc_dates_final, task="tcd"
                )
            ...
            s1_asc = data["s1_asc"]["img"][0].nan_to_num()
            ...
            encoded_patch["s1_asc_lat_mu"] = np.zeros((10, 64, 768, 768))
            ...
            encoded_patch["s1_asc_lat_mu"] = encoded_patch["s1_asc_lat_mu"][
                :, :, margin:-margin, margin:-margin
            ]

            encoded_patch["s1_asc_doy"] = np.arange(
                encoded_patch["s1_asc_lat_mu"].shape[0]
            )
            ...
            torch.save(
                {
                    "img": torch.Tensor(encoded_patch["s1_asc_lat_mu"]),
                    "matrix": {
                        "x_min": xy_matrix[0, 0].min(),
                        "x_max": xy_matrix[0, 0].max(),
                        "y_min": xy_matrix[1, :, 0].min(),
                ...
                os.path.join(output_folder, sat, f"s1_asc_{enum}.pt"),
            )
