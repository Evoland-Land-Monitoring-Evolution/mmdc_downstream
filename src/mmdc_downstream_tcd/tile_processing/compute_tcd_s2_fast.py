...
def get_s2_gt(folder_data: str | Path, months_folders: list[str | int], window: int):
    ...
    log.info("Sliced list")

    log.info(slices_list)
    ...
    for sliced in slices_list:
        ...
        if len(tcd_values_sliced) > 0:
            ...
            sliced_modality = get_sliced_modality(
                ...
                meteo=False,
                coord=[
                    np.unique(tcd_values_sliced["x_round"].values),
                    np.flip(np.unique(tcd_values_sliced["y_round"].values)),
                ],
                dates=s2_dates,
            )
            ...
            _, data, dates = get_s2(
                s2_dates, temp_slice, data, images, sliced_modality_selected
            )
            ...
            x_row = np.unique(sliced_modality_selected.x.values)
            y_col = np.flip(np.unique(sliced_modality_selected.y.values))

            x_coords = np.repeat([x_row], repeats=len(y_col), axis=0)
            y_coords = np.repeat(y_col.reshape(-1, 1), repeats=len(x_row), axis=1)
            xy_matrix_small = np.stack([x_coords, y_coords])
...
gt_df.to_csv(os.path.join(folder_data, "gt_tcd_t32tnt.csv"))
