...
for sat in satellites:
    ...
    all_points_csv = np.array([csv for csv in csv_files if csv.endswith(f"{sat}.csv")])
    all_points_csv = all_points_csv[0] if len(all_points_csv) > 0 else None
    if all_points_csv is not None:
        data = pd.read_csv(os.path.join(csv_folder, all_points_csv))
        days_name = all_points_csv.replace("encoded", "days").replace("csv", "npy")
