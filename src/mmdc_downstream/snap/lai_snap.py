...
class BVNET(nn.Module):
    ...
    def save_weights(self, dir, prefix=""):
        ...
        pd.DataFrame(weights_1).to_csv(os.path.join(
            dir, f"{prefix}{self.ver}_{self.variable}_weights_1.csv"),
                                       header=False,
                                       index=False,
                                       mode="w")
        pd.DataFrame(weights_2).to_csv(os.path.join(
            dir, f"{prefix}{self.ver}_{self.variable}_weights_2.csv"),
                                       header=False,
                                       index=False,
                                       mode="w")
        pd.DataFrame(bias_1).to_csv(os.path.join(
            dir, f"{prefix}{self.ver}_{self.variable}_bias_1.csv"),
                                    header=False,
                                    index=False,
                                    mode="w")
        pd.DataFrame(bias_2).to_csv(os.path.join(
    ...
    def get_batch_loss(self, batch, loc_bv=0, scale_bv=1):
        ...
        return ((variable_pred - loc_bv) / scale_bv - (
            (variable.to(self.device) - loc_bv) / scale_bv)).pow(2).mean()
...
#     viewZen_norm = normalize(sample[:, 8].unsqueeze(1), norm_factors["min_sample_viewZen"],
#                              norm_factors["max_sample_viewZen"])
#     sunZen_norm = normalize(sample[:, 9].unsqueeze(1), norm_factors["min_sample_sunZen"],
...
#         assert torch.isclose(n_snap_nn, torch.cat((neuron1, neuron2, neuron3, neuron4, neuron5), axis=1),
#                              atol=1e-4).all()
#
