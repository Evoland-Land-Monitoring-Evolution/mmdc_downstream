...
for i in range(nrows):
    for j in range(ncols):
        ...
        axis.set_xlabel("loss=" + str(np.round(loss_fn(torch.Tensor(pred), torch.Tensor(y_test)).item(), 4)))
...
#
# # Predict
# y_1_n = denormalize(regr_1_n.predict(normalize(X_test, input_min, input_max)), lai_min, lai_max)
# y_2_n = denormalize(regr_2_n.predict(normalize(X_test, input_min, input_max)), lai_min, lai_max)
