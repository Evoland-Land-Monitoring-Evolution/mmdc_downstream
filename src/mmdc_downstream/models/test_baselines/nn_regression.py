...
# Non-normalized data
mlp = neural_network.MLPRegressor(hidden_layer_sizes=(5), activation='tanh', solver='adam', alpha=0.01, batch_size=1*64*64,
                                  learning_rate='constant', learning_rate_init=0.0001, max_iter=2000,
                                  shuffle=True, tol=0.001, verbose=True,
                                  early_stopping=True, validation_fraction=0.1, n_iter_no_change=20)

mlp.fit(normalize(X_train, input_min, input_max),
...
# Predict
# pred = denormalize(mlp.predict(normalize(X_test, input_min, input_max)), lai_min, lai_max)
...
plt.xlabel("loss=" + str(np.round(loss_fn(torch.Tensor(pred), torch.Tensor(y_test)).item(), 4)))
plt.savefig("mlp.png")
plt.show()
