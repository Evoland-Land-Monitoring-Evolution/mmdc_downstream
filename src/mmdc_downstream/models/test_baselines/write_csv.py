...
class MMDCDownstreamRegressionLitModule(MMDCDownstreamBaseLitModule):
    ...
    def step(self, batch: Any, stage: str = "train") -> Any:
        ...
        s2_ref = rearrange(batch.s2_x, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy()/10000
        s2_angles = rearrange(batch.s2_a, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy()
        s1 = standardize_data(rearrange(batch.s1_x, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy(),
                shift=self.stats.sen1.shift.type_as(
                    batch.s1_x),
                scale=self.stats.sen1.shift.type_as(
                    batch.s1_x))
        s1_angles = rearrange(batch.s1_a, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy()
        s1_mask = rearrange(batch.s1_vm, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy()
        s2_input = rearrange(reg_input, 'b c h w -> (b h w) c')[~mask][idx].cpu().numpy()
        latent = self.model_mmdc.get_latent_mmdc(batch)
        s1_lat = rearrange(latent.latent_S1_mu, 'b c h w -> (b h w) c')[~mask][idx].to(
            torch.float32).cpu().numpy()
        s2_lat = rearrange(latent.latent_S2_mu, 'b c h w -> (b h w) c')[~mask][idx].to(
            torch.float32).cpu().numpy()
        exp_lat = rearrange(latent.latent_experts_mu, 'b c h w -> (b h w) c')[~mask][idx].to(
            torch.float32).cpu().numpy()
        gt = lai_gt.reshape(-1)[~mask][idx].to(torch.float32).unsqueeze(-1).cpu().numpy().round(3)
        ...
        data_to_write = np.hstack((s2_ref, s2_angles, s1, s1_angles, s1_mask, s2_input,
                                   s1_lat, s2_lat, exp_lat, gt))
