...
df = df.rename(columns={'s1_0': 's1_asc_0', 's1_1': 's1_asc_1', 's1_2': 's1_asc_2', 's1_3': 's1_desc_3', 's1_4': 's1_desc_4', 's1_5': 's1_desc_5', 's1_mask_0': 's1_mask_asc', 's1_mask_1': 's1_mask_desc'})
...
train_col_all = [i for i in df.columns
...
for data_type in train_data:
    if data_type != "s1_mask":
        ...
        if train_col[0].startswith('s1_asc'):
            ...
            X_train_all, X_test_all, y_train, y_test = X_train_all[~mask_train], X_test_all[~mask_test], y_train[~mask_train], y_test[~mask_test]
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=64*64, shuffle=True, num_workers=2)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=100*64*64, shuffle=False, num_workers=2)
        ...
        for epoch in range(n_epochs):  # 5 epochs at maximum
            ...
            print('Train Loss after epoch %5d: %.3f' %
                  (epoch + 1, current_loss_train/int(len(train_dataset)/train_dataloader.batch_size)))
            ...
            print('Test Loss after epoch %5d: %.3f' %
                  (epoch + 1, current_loss_test / int(len(test_dataset)/test_dataloader.batch_size)))
            ...
            plt.xlabel("loss="+str(np.round(loss_fn(torch.Tensor(pred), y_test_denorm).item(), 4)))
