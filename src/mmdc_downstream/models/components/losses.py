...
def mask_and_flatten_s1(data: torch.Tensor,
    ...
    asc_data = (asc_data.flatten()[asc_mask.flatten()].reshape(s1_bands,
                                                               -1).transpose(
                                                                   0, 1))
    desc_data = (desc_data.flatten()[desc_mask.flatten()].reshape(
        s1_bands, -1).transpose(0, 1))

    return torch.cat([asc_data, desc_data], dim=0)


def mask_and_flatten_s2(data: torch.Tensor,
                        mask: torch.Tensor) -> torch.Tensor:
    """Mask the S1 data tensor and flatten the spatial dimensions"""
...
def mask_data(data: torch.Tensor,
...
def mask_s1_like(data: torch.Tensor,
                 mask: torch.Tensor) -> tuple[torch.Tensor, float]:
    """Mask data with S1 shape for loss computation"""
    s1_bands = data.shape[1] // 2
    s1_x = data.clone()
    s1_x[:, :s1_bands,
         ...] = s1_x[:, :s1_bands, ...] * (1 - mask[:, 0, ...]).unsqueeze(1)
    s1_x[:, s1_bands:,
         ...] = s1_x[:, s1_bands:, ...] * (1 - mask[:, 1, ...]).unsqueeze(1)
    valid_ratio = 1.0 - mask.sum() / (mask.shape[2] * mask.shape[3] * 2)
    return s1_x, valid_ratio


def mask_s2_like(data: torch.Tensor,
                 mask: torch.Tensor) -> tuple[torch.Tensor, float]:
